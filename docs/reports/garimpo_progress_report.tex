...
in practice. Figure~\ref{fig:mapinterface} shows the interface on a
reference-date scene (2020-10-08, path/row 226/121), for which exact
footprint coordinates are available.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig_map_interface.png}
\caption{Prototype map interface, showing the confidence-coded detection
markers and threshold slider on a reference-date scene. Markers that
appear visually distant from the main river channel at this zoom level
were investigated (Section~\ref{sec:falsepos}) and found, on closer
inspection, to sit near small tributary streams not obvious at this scale
--- consistent with the water-adjacency finding discussed there, not an
inconsistency with it.}
\label{fig:mapinterface}
\end{figure}
